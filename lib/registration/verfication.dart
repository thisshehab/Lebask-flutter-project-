...
import 'package:lebask/main/home/locations/addlocation2.dart';
...
class verfication extends StatefulWidget {
  ...
  // const verfication({required this.verificationId});
...
class _verficationState extends State<verfication> {
  ...
  addtheuser() async {
    Reference refstore =
        FirebaseStorage.instance.ref('/requsts/${widget.imagename}');
    ...
    if (widget.address == null) {
      ...
      userref.add({
        ...
        prefs.setString("userphone", widget.phonenumber);
        userid = value.id;
        ...
        FirebaseFirestore.instance
          ...
          //     .doc(value2.id)
          //     .collection('products')
          //     .add({'itemid': ''}).then((value3) => FirebaseFirestore.instance
          //         .collection("users")
          //         .doc(value.id)
          //         .collection('orders')
          //         .doc(value2.id)
          //         .collection('products')
          //         .doc(value3.id)
          //         .update({'docid': value3.id}));
        });
      });
    } else {
    await refstore.putFile(widget.picture!);
    Future<String> imagename = refstore.getDownloadURL();
    CollectionReference userref =
        FirebaseFirestore.instance.collection("sillers");
    userref.add({
      'name': widget.name,
      'address': widget.address,
      'phone': widget.phonenumber,
      'logo': await imagename,
      'sold': '0',
      "silles":"10",
      "location":
          "${addlocation2State.selectedlatitude},${addlocation2State.selectedlongtude}"
    }).then((value) {
      FirebaseFirestore.instance.doc(value.path).update({"docid": value.id});
      //save the information local
      prefs.setString("sillerid", value.id);
      prefs.setString("sillername", widget.name);
      prefs.setString("silleraddress", widget.address!);
      prefs.setString("sillerphone", widget.phonenumber);
      //save the information in cloud
    });
    }
  }
